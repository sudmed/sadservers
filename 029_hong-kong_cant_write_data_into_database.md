...
**Test:** `sudo -u postgres psql -c "insert into persons(name) values ('jane smith');" -d dt` should return `INSERT 0 1`.  
...
● postgresql.service - PostgreSQL RDBMS
   ...
   Active: active (exited) since Fri 2024-02-09 10:37:38 UTC; 3min 17s ago
  Process: 612 ExecStart=/bin/true (code=exited, status=0/SUCCESS)
 Main PID: 612 (code=exited, status=0/SUCCESS)

Feb 09 10:37:38 ip-172-31-25-11 systemd[1]: Starting PostgreSQL RDBMS...
Feb 09 10:37:38 ip-172-31-25-11 systemd[1]: Started PostgreSQL RDBMS.
```

`cat /var/log/postgresql/postgresql-14-main.log.1`  
```console
2022-08-31 17:04:00.645 UTC [589] FATAL:  could not create lock file "postmaster.pid": No space left on device
pg_ctl: could not start server
Examine the log output.
```

##### We can see an error `No space left on device`. Let's check config, space and disks.
...
udev               56K   297   56K    1% /dev
tmpfs              59K   397   58K    1% /run
/dev/nvme1n1p1    504K   39K  466K    8% /
tmpfs              59K     1   59K    1% /dev/shm
tmpfs              59K     2   59K    1% /run/lock
tmpfs              59K    17   59K    1% /sys/fs/cgroup
/dev/nvme1n1p15      0     0     0     - /boot/efi
```
...
##### There is no directory `main` in `/opt/pgdata/`. Let's check fstab
`cat /etc/fstab`  
```console
...
```console
NAME         MAJ:MIN RM  SIZE RO TYPE MOUNTPOINT
nvme0n1      259:0    0    8G  0 disk /opt/pgdata
nvme1n1      259:1    0    8G  0 disk 
├─nvme1n1p1  259:2    0  7.9G  0 part /
├─nvme1n1p14 259:3    0    3M  0 part 
└─nvme1n1p15 259:4    0  124M  0 part /boot/efi
...
```console
NAME         FSTYPE LABEL UUID                                 FSAVAIL FSUSE% MOUNTPOINT
nvme0n1      xfs          9a2e1bf9-50e8-41a9-9f8c-5dd837869c50     28K   100% /opt/pgdata
nvme1n1                                                                       
├─nvme1n1p1  ext4         5db68868-2d70-449f-8b1d-f3c769ec01c7    6.1G    15% /
├─nvme1n1p14                                                                  
└─nvme1n1p15 vfat         72C9-F191                             123.5M     0% /boot/efi
...
```console
total 8
drwxr-xr-x  3 postgres postgres   18 Feb  9 13:16 .
drwxr-xr-x  3 root     root     4096 May 21  2022 ..
drwx------ 19 postgres postgres 4096 May 21  2022 main
```
...
● postgresql.service - PostgreSQL RDBMS
   Loaded: loaded (/lib/systemd/system/postgresql.service; enabled; vendor preset: enabled)
   Active: active (exited) since Fri 2024-02-09 13:19:31 UTC; 6s ago
  Process: 1443 ExecStart=/bin/true (code=exited, status=0/SUCCESS)
 Main PID: 1443 (code=exited, status=0/SUCCESS)

Feb 09 13:19:31 ip-172-31-25-11 systemd[1]: Starting PostgreSQL RDBMS...
Feb 09 13:19:31 ip-172-31-25-11 systemd[1]: Started PostgreSQL RDBMS.
```
