...
##### Let's add any other additional code to keep the process running, so the container continues to exist and doesn't exit. For example, we will use ~~an infinite `while` loop with `sleep` command~~ black hole.
